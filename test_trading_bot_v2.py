...
#df = pd.read_csv("./Stock Evolution/data/quote/ALO.PA.csv")
...
class TradingFees:
    def __init__(self, fee_type, fee, broker):
        if self.check_fee_type(fee_type):
            self.fee_type = fee_type

    def check_fee_type(self, fee_type):
        if fee_type not in ['amount', "percent"]:
            return False
        else:
            return True
...
CASH_ON_HAND = 1000
ORDER_TO_MANAGE = 10
LOW_LIMIT_PERCENT = 50
HIGH_LIMIT_PERCENT = 75
...
for current_date in pd.date_range(datetime(2024, 7, 8), datetime(2024, 9, 25)):
    ...
    if current_date.weekday() not in [5, 6]:
        ...
        if current_date.weekday() == 0:
            if last_trade == None or last_trade == "sell":
                if quote < 17.00:
                    alo_ord = Order(alo)
...
tf = TradingFees("amount", 2, "Fortuneo Starter")
